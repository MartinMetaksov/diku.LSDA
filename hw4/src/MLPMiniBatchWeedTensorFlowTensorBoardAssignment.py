...
dataTrain = tf.contrib.learn.datasets.base.load_csv_without_header(
    filename=FLAGS.data_dir + '/LSDA2017WeedCropTrain.csv', target_dtype=np.int, features_dtype=np.float32,
    target_column=-1)
dataTest = tf.contrib.learn.datasets.base.load_csv_without_header(filename=FLAGS.data_dir + '/LSDA2017WeedCropTest.csv',
                                                                  target_dtype=np.int, features_dtype=np.float32,
                                                                  target_column=-1)
dataValidate = tf.contrib.learn.datasets.base.load_csv_without_header(
    filename=FLAGS.data_dir + '/LSDA2017WeedCropValidation.csv', target_dtype=np.int, features_dtype=np.float32,
    target_column=-1)
...
for i in range(FLAGS.maxIter):
    ...
    if (i + 1) % 100 == 0:
        ...
        currentValidation, summary = sess.run([accuracy, merged], feed_dict={x_data: dataValidate.data,
...
print("final training accuracy:",
      sess.run(accuracy, feed_dict={x_data: dataTrain.data, y_target: np.transpose([dataTrain.target])}),
      "final test accuracy: ",
      sess.run(accuracy, feed_dict={x_data: dataTest.data, y_target: np.transpose([dataTest.target])}),
      "final validation accuracy: ",
      sess.run(accuracy, feed_dict={x_data: dataValidate.data, y_target: np.transpose([dataValidate.target])}))
