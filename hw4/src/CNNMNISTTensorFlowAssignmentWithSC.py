...
#4x4 max pool, parallel to the first 2x2 max pool
h_pool3=max_pool_4x4(h_conv1) #max pooling, resulting feature maps are 7X7
...
#concat output of the 4x4 max pool with the output of the 2nd 2x2 max pool, after flattening 
print(tf.shape(h_pool2))
h_pool3_flat=tf.reshape(h_pool3,[-1, 7*7*32]) #flatten all 32 output 7X7 maps from the 4x4 max pool layer
h_pool2_flat = tf.reshape(h_pool2,[-1, 7*7*64]) # flatten all 64 output 7x7 maps , 7*7*64 from the second 2x2 max pool layer
h_pool_concat=tf.concat([h_pool3_flat,h_pool2_flat],1)

# Fully connected layer
# W_fc1: [inputs, number of neurons]
W_fc1 = weight_variable([4704, 1024])  #(7*7*32)+(7*7*64)=4704
