...
def bias_variable(shape):
    initial = tf.constant(0.1, shape=shape) # positive value, trying to avoid "dead" ReLUs
    return tf.Variable(initial)
...
h_pool1sc = max_pool_4x4(h_conv1)  # for shortcut connections, max pooling, resulting feature maps are 7x7
...
h_pool1sc_flat = tf.reshape(h_pool1sc, [-1, 7*7*32]) # flatten all 32 output 7x7 maps

h_fc1 = tf.nn.relu(tf.matmul(tf.concat([h_pool2_flat, h_pool1sc_flat], 1), W_fc1) + b_fc1) # second argument of tf.concat indicates the axis
...
# Adam optimizer, default parameters learning_rate=0.001 (here overwritten), beta1=0.9, beta2=0.999, epsilon=1e-08
